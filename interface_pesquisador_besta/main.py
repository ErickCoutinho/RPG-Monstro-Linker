...
def pesquisar_monstro():
    ...
    if not resultado_pesquisa.empty:
        ...
        # Display Tagmar attributes
        resultado_text.insert(tk.END, f"\nTagmar:\nNome: {resultado_pesquisa['Tagmar'].values[0]}\nDescrição: {resultado_pesquisa['descrição'].values[0]}\n")
        for attribute, value in zip(['INT', 'AUR', 'CAR', 'FOR', 'FIS', 'AGI', 'PER'], resultado_pesquisa[['INT(tagmar)', 'AUR(tagmar)', 'CAR(tagmar)', 'FOR(tagmar)', 'FIS(tagmar)', 'AGI(tagmar)', 'PER(tagmar)']].values[0]):
            resultado_text.insert(tk.END, f"{attribute}: {value}\n")

        # Display Pathfinder attributes
        resultado_text.insert(tk.END, f"\nPathfinder:\nNome: {resultado_pesquisa['Pathfinder'].values[0]}\n")
        for attribute, value in zip(['Str', 'Dex', 'Con', 'Int', 'Wis', 'Cha'], resultado_pesquisa[['Str (Pathinder)', 'Dex (Pathinder)', 'Con (Pathinder)', 'Int (Pathinder)', 'Wis (Pathinder)', 'Cha (Pathinder)']].values[0]):
            resultado_text.insert(tk.END, f"{attribute}: {value}\n")

        # Display D&D attributes
        resultado_text.insert(tk.END, f"\nD&D:\nNome: {resultado_pesquisa['D&D'].values[0]}\n")
        for attribute, value in zip(['Str', 'Dex', 'Con', 'Int', 'Wis', 'Cha'], resultado_pesquisa[['Str (D&D)', 'Dex (D&D)', 'Con (D&D)', 'Int (D&D)', 'Wis (D&D)', 'Cha (D&D)']].values[0]):
            resultado_text.insert(tk.END, f"{attribute}: {value}\n")
...
def on_entry_click(event):
    if entry.get() == "Digite aqui!":
        entry.delete(0, "end")
        entry.insert(0, "Digite aqui!")  # Define a cor do texto para preto
...
janela = tk.Tk()
janela.title("RPG MONSTER LINKE R")
